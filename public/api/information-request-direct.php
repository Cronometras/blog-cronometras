<?php
// Endpoint directo para solicitud de información (sin slash final)
...
error_log("Information Request Direct API endpoint called with method: " . $_SERVER['REQUEST_METHOD']);
...
try {
    ...
    // Debug log
    error_log("Information request form data (direct): name=$name, email=$email, company=$company, phone=$phone, message=$message");
    ...
    // Log error but still return success to user
    error_log("Exception in information-request-direct.php: " . $e->getMessage());
    echo json_encode([
        'success' => true, // Still return success to user
        'message' => $successMsg,
        'debug' => [
