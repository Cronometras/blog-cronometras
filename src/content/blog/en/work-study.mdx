...
Frequency times are the times of elements that do not appear in every cycle, but only in some laps of the task. The classic time and motion study literature usually classifies these activities according to their regularity into three categories:
...
In CRONOMETRAS this concept is modeled with **frequent elements**: elements that appear once every x cycles, unlike repetitive elements (which appear every lap) and machine elements. For each element its **frequency** is also recorded, that is, the number of pieces resulting from each cycle, so that its time is distributed correctly across the cycles in which it does not intervene.
